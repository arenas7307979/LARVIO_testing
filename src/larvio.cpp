...
LarVio::LarVio(std::string &config_file_) : is_gravity_set(false),
                                            is_first_img(true),
...
{
  ScopedTrace tracer("LarVio::loadParameters");
...
bool LarVio::initialize()
{
  ...
  ScopedTrace tracer("LarVio::initialize");
                             ...
                             std::vector<ImuData> &imu_msg_buffer)
{
  ...
  ScopedTrace tracer("LarVio::processFeatures");
...
void LarVio::batchImuProcessing(const double &time_bound,
                                std::vector<ImuData> &imu_msg_buffer)
{
  ScopedTrace tracer("LarVio::batchImuProcessing");
...
{
  ScopedTrace tracer("LarVio::processModel");
...
void LarVio::predictNewState(const double &dt,
                             ...
                             const Vector3d &acc)
{
  ScopedTrace tracer("LarVio::predictNewState");
  ...
  if (gyro_norm > 1e-5)
  {
    dq_dt = (cos(gyro_norm * dt * 0.5) * Matrix4d::Identity() +
            ...
            q; // QXC：四元数的毕卡法更新，参见秦永元《惯性导航》第一版P301式(9.2.52)，注意角增量的计算为gyro_norm*dt
    dq_dt2 = (cos(gyro_norm * dt * 0.25) * Matrix4d::Identity() +
              1 / gyro_norm * sin(gyro_norm * dt * 0.25) * Omega) *
             q; // QXC：dt/2时的四元数毕卡法更新
  }
  else
  { // QXC：当角增量很小时的近似，实部项没有做近似，虚部项使用了洛必达法则！
...
{
  ScopedTrace tracer("LarVio::stateAugmentation");
...
{
  ScopedTrace tracer("LarVio::addFeatureObservations");
...
{
  ScopedTrace tracer("LarVio::measurementJacobian_msckf");
  ...
  Matrix3d R_b2w = Quaterniond(
                       q_imu(3), q_imu(0), q_imu(1), q_imu(2))
                       .toRotationMatrix();
  Matrix3d R_w2b = R_b2w.transpose();
...
{
  ScopedTrace tracer("LarVio::featureJacobian_msckf");
...
void LarVio::measurementJacobian_ekf_3didp(const StateIDType &state_id,
                                           ...
                                           Eigen::Vector2d &r)
{
  ScopedTrace tracer("LarVio::measurementJacobian_ekf_3didp");
  // Prepare all the required data.
  ...
  Matrix3d R_bk2w = Quaterniond(
                        q_imu_k(3), q_imu_k(0), q_imu_k(1), q_imu_k(2))
                        .toRotationMatrix();
  ...
  Matrix3d R_ba2w = Quaterniond(
                        q_imu_a(3), q_imu_a(0), q_imu_a(1), q_imu_a(2))
                        .toRotationMatrix();
  ...
  if (anch_is_nui)
  {
    ...
    Matrix3d R_ca2w = Quaterniond(
                          q_cam_a(3), q_cam_a(0), q_cam_a(1), q_cam_a(2))
                          .toRotationMatrix();
...
void LarVio::measurementJacobian_ekf_1didp(const StateIDType &state_id,
                                           ...
                                           Eigen::Vector2d &r)
{
  ScopedTrace tracer("LarVio::measurementJacobian_ekf_1didp");
  // Prepare all the required data.
  if (state_server.imu_states_augment.find(state_id) == state_server.imu_states_augment.end())
    printf("state_id is not in imu_states_augment !!!!!");
  IMUState_Aug imu_state_k = state_server.imu_states_augment[state_id];
...
{
  ScopedTrace tracer("LarVio::featureJacobian_ekf_new");
...
{
  ScopedTrace tracer("LarVio::featureJacobian_ekf");
  auto &feature = map_server[feature_id];
...
{
  ScopedTrace tracer("LarVio::measurementUpdate_msckf");
       ...
       ++i, feature_itr++)
  {
    ...
    Matrix3d R_c2w = Quaterniond(
                         cam_qua(3), cam_qua(0), cam_qua(1), cam_qua(2))
                         .toRotationMatrix();
...
{
  ScopedTrace tracer("LarVio::measurementUpdate_hybrid");
...
{
  ScopedTrace tracer("LarVio::findRedundantImuStates");
  ...
  const Matrix3d key_rotation = Quaterniond(
                                    key_qua(3), key_qua(0), key_qua(1), key_qua(2))
                                    .toRotationMatrix();
...
{
  ScopedTrace tracer("LarVio::pruneImuStateBuffer");
        ...
        else
        {
          ...
          if (3 == feature_idp_dim)
          {
            ...
            Matrix3d R_c2w_new = Quaterniond(
                                     cam_qua_new(3), cam_qua_new(0), cam_qua_new(1), cam_qua_new(2))
                                     .toRotationMatrix();
...
{
  ScopedTrace tracer("LarVio::resetFejPoint");
...
{
  ScopedTrace tracer("LarVio::resetFejPoint");
  if (coarse_feature_dis.empty() || coarse_feature_dis.size() < 20)
  {
...
{
  ScopedTrace tracer("LarVio::measurementUpdate_ZUPT_vpq");
  ...
  // r_q
  const Vector4d &q_c = state_server.imu_states_augment[state_server.imu_state.id].orientation;
  Quaterniond q_curr(q_c(3), q_c(0), q_c(1), q_c(2));
  const Vector4d &q_p = state_server.imu_states_augment[state_server.imu_state.id - 1].orientation;
                                    ...
                                    const StateIDType &old_state_id, const StateIDType &new_state_id)
{
  ...
  Matrix3d R_b2w_old = Quaterniond(
                           q_imu_old(3), q_imu_old(0), q_imu_old(1), q_imu_old(2))
                           .toRotationMatrix();
  ...
  Matrix3d R_c2w_old = Quaterniond(
                           cam_qua_old(3), cam_qua_old(0), cam_qua_old(1), cam_qua_old(2))
                           .toRotationMatrix();
  ...
  Matrix3d R_b2w_new = Quaterniond(
                           q_imu_new(3), q_imu_new(0), q_imu_new(1), q_imu_new(2))
                           .toRotationMatrix();
...
void LarVio::updateFeatureCov_1didp(const FeatureIDType &feature_id,
                                    const StateIDType &old_state_id, const StateIDType &new_state_id)
{
  ScopedTrace tracer("LarVio::updateFeatureCov_1didp");
  const size_t state_size = state_server.imu_states_augment.size();
  const size_t feature_size = state_server.feature_states.size();
...
void LarVio::rmLostFeaturesCov(const std::vector<FeatureIDType> &lost_ids)
{
  if (lost_ids.empty())
    return;
  ScopedTrace tracer("LarVio::rmLostFeaturesCov");
...
{
  ScopedTrace tracer("LarVio::getNewAnchorId");
...
{
  ScopedTrace tracer("LarVio::rmUselessNuisanceState");
